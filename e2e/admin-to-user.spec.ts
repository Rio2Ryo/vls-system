...
test.describe("Admin → User Integration", () => {
  ...
  test("2: Admin-registered company CM video plays in STEP 2", async ({ page }) => {
    ...
    // Tag input is now a click-to-expand dropdown
    ...
    const testCompany = companies.find((c: { name: string }) => c.name === "テストCM企業");
    expect(testCompany).toBeTruthy();
    expect(testCompany.videos.cm15).toBe("L_jWHffIx5E");

    // Set session and go to STEP 2
    await page.evaluate(() => {
      const stored = JSON.parse(localStorage.getItem("vls_admin_companies") || "[]");
      const found = stored.find((c: { name: string }) => c.name === "テストCM企業");
      if (!found || found.videos.cm15 !== "L_jWHffIx5E") {
  ...
  test("3: Admin-uploaded photos appear in STEP 3 with watermarks", async ({ page }) => {
    await adminLogin(page);
    await page.locator('[role="tablist"]').getByRole("button", { name: /写真管理/ }).click();
  ...
  test("5: Full Admin E2E: login -> event -> photos -> CM -> URL -> user access", async ({ page }) => {
    ...
    await page.getByText("タグを選択（クリックして展開）").click();
    await page.getByRole("button", { name: "教育" }).click();
    await page.getByRole("button", { name: "テクノロジー" }).click();
